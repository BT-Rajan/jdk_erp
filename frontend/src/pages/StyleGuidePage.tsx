...
/** A living, routed reference for every component in `components/ui`,
 * `components/forms` and `components/charts` (docs/modules/common_ui_components.md)
 * -- composed on one page so a change to a shared component is visually
 * verifiable in one place rather than hunting across business pages
 * that don't exist yet. Rendered inside AppLayout like any other route,
 * so the Sidebar/TopNav chrome around it is the real one, not a demo
 * of its own. */
export function StyleGuidePage() {
  ...
  return (
    <div className="space-y-10">
      <Breadcrumbs items={[{ label: 'Home', to: '/' }, { label: 'Style guide', to: '/styleguide' }, { label: 'Acme Corp' }]} />
