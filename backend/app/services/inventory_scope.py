...
Opening Stock, ledger/balance reconciliation reporting) -- the same
...
authorization). `adjust`, `opening_stock` and `reconcile` are
deliberately three separate actions within this one module (not one
shared action) so an organisation can grant them independently -- e.g. a
